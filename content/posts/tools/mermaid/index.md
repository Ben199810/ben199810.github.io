...
draft: false
tags: ["Mermaid"]
description: "Mermaid 是一個開源的圖表繪製工具，很多的圖表都可以用文字的方式來描述，Mermaid 會自動將文字轉換成圖表。可以方便我們快速的建立流程圖、循序圖、甘特圖、類別圖、狀態圖、ER 圖、圓餅圖、心智圖等，修改起來也很方便，也可以使用版本控制管理每一次的歷史修改。"
---

## Mermaid 介紹

在各種專案管理、系統設計、流程規劃中，常常需要用圖表來呈現資訊。可以幫助我們更清楚地了解整個流程和各個環節之間的關係。有些工具可以繪製圖表，但需要使用滑鼠拖拉，操作起來比較繁瑣。而且對於系統設計的異動或流程規劃的調整，每次都要重新繪製圖表，效率不高。這時候就可以使用 Mermaid 這個工具。

Mermaid 是一個開源的圖表繪製工具，很多的圖表都可以用文字的方式來描述，Mermaid 會自動將文字轉換成圖表。可以方便我們快速的建立流程圖、循序圖、甘特圖、類別圖、狀態圖、ER 圖、圓餅圖、心智圖等，修改起來也很方便，也可以使用版本控制管理每一次的歷史修改。
...
適合用於腦力激盪、知識整理、專案規劃等，將想法以樹狀結構呈現，方便快速捕捉和組織資訊。
...
- mindmap：宣告心智圖
- root((文字))：宣告根節點，使用雙括號表示圓
- 子節點：直接縮排表示層級關係，越往下表示層級越深
